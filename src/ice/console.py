...
class Console(BackedObject):
    def __init__(self, backing_store, identifier, config, filesystem):
    ...
    def find_roms(self):
        ...
        """
        roms = []
        if not self.filesystem.path_exists(self.roms_directory()):
        ...
        for file_path in self.filesystem.files_in_directory(self.roms_directory()):
