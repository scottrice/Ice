...
class Console(BackedObject):
    ...
    def is_valid_rom(self, path):
        ...
        If a list of extensions is supplied for this console, we check if the
            path has a valid extension.
        If no extensions are defined for this console, we just accept any file.
        Hidden files are ignored.
        """

        if os.path.basename().startswith("."):
            # hidden file
            return False
        ...
        return any(extension == ('.' + x.strip().lower()) for x in\
            self.extensions.split(','))

    def find_roms(self):
        """
        Build a list of all the ROMs from the appropriate directory for the
        console
        """
        roms = []
        for root, _, files in os.walk(self.roms_directory(), followlinks=True):
            for file_name in files:
                file_path = os.path.join(root, file_name)
                if self.emulator is not None and self.is_valid_rom(file_path):
                    # append valid ROMs to rom list, skip other files
