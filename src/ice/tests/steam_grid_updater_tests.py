...
class SteamGridUpdaterTests(unittest.TestCase):
  def setUp(self):
    ...
    self.updater = steam_grid_updater.SteamGridUpdater(self.mock_provider, self.mock_logger)
  ...
  def test_updater_does_nothing_if_provider_throws_an_error(self):
    self.mock_provider.image_for_rom.side_effect = provider_error.ProviderError("Random provider error")
    shortcut = mock.MagicMock()
    shortcut.custom_image.return_value = None
    rom = mock.MagicMock()
    rom.to_shortcut.return_value = shortcut
