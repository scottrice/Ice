...
class Filesystem(object):
  ...
  def is_directory(self, path):
    assert self.path_exists(path), "`is_directory` expects the given path to exist"
    return os.path.isdir(path)

  def _paths_in_directory(self, directory):
    assert self.is_directory(directory), "`paths_in_directory` expects a directory parameter"
